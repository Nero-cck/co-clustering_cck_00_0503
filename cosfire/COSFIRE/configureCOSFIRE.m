...
%featureImage：line1
%keypoint：round([y x])
%params: symm_params/asymm_params

if strcmp(params.inputfilter.name,'Gabor')
    % Get responses of a Bank of Gabor filters得到Gabor滤波器组的响应
...
elseif strcmp(params.inputfilter.name,'DoG')
    % Get responses of a Bank of DoG filters得到DOG过滤器的回应
...
%weighted geometric mean params
maxrho = max(params.COSFIRE.rholist);%取最大参数
