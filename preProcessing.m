function [img_pre,mask] = preProcessing(img0,mask)
...
img_pre = fakepad(G,mask);
se = strel('disk',5);
mask = imerode(mask,se); 
% [~,img_pre] = getBigimg(G, mask);
 img_pre = adapthisteq(uint8(img_pre),'NumTiles',[8,8],'clipLimit',0.01);
end
